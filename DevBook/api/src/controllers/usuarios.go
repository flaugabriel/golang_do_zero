...
func AtualizarUsuario(w http.ResponseWriter, r *http.Request) {
	...
	if usuarioId != usuarioIdNoToken {
		respostas.Erro(w, http.StatusForbidden, errors.New("Não e possivel atualizar um usuario que não é o seu"))
		return
	}
...
func BuscarSeguidores(w http.ResponseWriter, r *http.Request) {
	...
	parametros := mux.Vars(r)
	usuarioIDParam, erro := strconv.ParseUint(parametros["usuarioId"], 10, 64)
	if erro != nil {
		respostas.Erro(w, http.StatusBadRequest, erro)
		return
	}
	if usuarioID != usuarioIDParam {
		respostas.Erro(w, http.StatusForbidden, errors.New("não é possível ver os seguidores de outro usuário"))
		return
	}
	db, erro := banco.Conectar()
	if erro != nil {
...
func AtualizarSenha(w http.ResponseWriter, r *http.Request) {
	...
	defer db.Close(context.Background())
	repositorio := repositorios.NovoRepositorioDeUsuarios(db)
	senhaAtualNoBanco, erro := repositorio.BuscarSenha(usuarioId)
	if erro != nil {
		respostas.Erro(w, http.StatusInternalServerError, erro)
		return
	}

	if erro = seguranca.VerificarSenha(senhaAtualNoBanco, senha.Atual); erro != nil {
		respostas.Erro(w, http.StatusUnauthorized, errors.New("Asenha atual não condiz com a senha que foi digitada"))
		return
	}
